...
if (!class_exists('qode_type1_walker_nav_menu')) {
	...
	// add classes to ul sub-menus
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($qode_options_elision['page_transitions'] == "0")):
			...
			if($item->hide == ""){
				...
				$item_output .= '<i class="q_menu_arrow fa fa-angle-right"></i>';
				$item_output .= '</a>';
			}
