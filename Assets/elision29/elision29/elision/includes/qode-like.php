...
class QodeLike {
	...
	function like_post($post_id, $action = 'get'){
		...
		switch($action) {
			...
			case 'get':
				...
				return '<i class="fa fa-heart fa-2x"></i><span class="qode-like-count">'. $like_count .'</span>';
				break;
	...
	function add_qode_like_portfolio_list(){
		...
		return '<a '.$font_size.' class="'. $class .'" id="qode-like-'. $portfolio_project_id .'" title="'. $title .'">'. $output .'</a>';
	}
}
