...
if(!function_exists('qode_header_meta')) {
	...
	function qode_header_meta() {
		...
		if(isset($qode_options_elision['disable_qode_seo']) && $qode_options_elision['disable_qode_seo'] == 'no') {
...
if(!function_exists('qode_wp_title')) {
	...
	function qode_wp_title($title, $sep) {
		...
		if(qode_seo_plugin_installed()) {
			...
			//is qode seo enabled?
			if(isset($qode_options_elision['disable_qode_seo']) && $qode_options_elision['disable_qode_seo'] !== 'yes') {
...
if(!function_exists('qode_ajax_meta')) {
	...
	function qode_ajax_meta() {
		...
		<?php } else if($qode_options_elision['meta_description']){?>
			<div class="seo_description"><?php echo $qode_options_elision['meta_description']; ?></div>
		<?php } ?>
		<?php if($seo_keywords !== ''){ ?>
			<div class="seo_keywords"><?php echo $seo_keywords; ?></div>
		<?php }else if($qode_options_elision['meta_keywords']){?>
			<div class="seo_keywords"><?php echo $qode_options_elision['meta_keywords']; ?></div>
		<?php }
	}
